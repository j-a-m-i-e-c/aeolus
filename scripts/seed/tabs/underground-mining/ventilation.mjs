...
export default function VentilationControl(aeolus: CustomComponentProps) {
  ...
  return <div style={{padding:11,minHeight:"100%",background:"linear-gradient(180deg,#0A0D0F,#07090B)",color:"#EDF2F4"}}>
    <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><div><div style={{fontSize:12,fontWeight:900}}>VENTILATION CONTROL</div><div style={{fontSize:7.5,color:"#717B81",marginTop:2}}>Atmospheric demand → primary + booster fans → measured airflow</div></div><div style={{textAlign:"right"}}><div style={{fontSize:9,fontWeight:850,color}}>{manual?"MANUAL BOOST":boost?"AUTO BOOST":"DEMAND AUTO"}</div><div style={{fontSize:7,color:"#69747A"}}>{airflow} m³/s</div></div></div>
    <div style={{border:"1px solid #30363A",borderRadius:10,background:"#0B0E10",padding:7}}><svg width="100%" height="155" viewBox="0 0 440 155">
      ...
      <g transform="translate(78 78)"><circle r="26" fill="#121619" stroke={color}/><g style={{transform:"rotate("+(phase*(4+demand*.09))+"deg)",transformOrigin:"0px 0px"}}>{[0,90,180,270].map(a=><path key={a} d="M0 0 C9 -10 17 -8 19 -3 C13 3 7 5 0 0 Z" fill={color} transform={"rotate("+a+")"}/>)}</g><text x="0" y="42" textAnchor="middle" fill="#8B959A" fontSize="7">PRIMARY {rpm}</text></g>
      <g transform="translate(295 78)"><circle r="19" fill="#121619" stroke="#E0A34A"/><g style={{transform:"rotate("+(phase*(3+demand*.08))+"deg)",transformOrigin:"0px 0px"}}><line x1="-12" y1="0" x2="12" y2="0" stroke="#E0A34A" strokeWidth="3"/><line x1="0" y1="-12" x2="0" y2="12" stroke="#E0A34A" strokeWidth="3"/></g><text x="0" y="35" textAnchor="middle" fill="#8B7B61" fontSize="7">BOOSTER {booster}</text></g>
      <text x="186" y="22" fill="#707A80" fontSize="7">RETURN AIRWAY</text><text x="350" y="104" fill="#707A80" fontSize="7">EXHAUST</text>
    </svg></div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginTop:7}}><div style={{border:"1px solid #30363A",borderRadius:8,padding:7,background:"#0B0E10"}}><div style={{fontSize:6.5,color:"#707A80"}}>ATMOSPHERIC REQUEST</div><div style={{fontSize:17,fontFamily:"monospace",fontWeight:800,color:requested>=80?"#F0B85D":"#B9C7CC",marginTop:2}}>{Math.round(requested)}%</div><div style={{fontSize:7,color:"#626D72"}}>{severity.toUpperCase()} from Atmospheric Safety</div></div><div style={{border:"1px solid #30363A",borderRadius:8,padding:7,background:"#0B0E10"}}><div style={{fontSize:6.5,color:"#707A80"}}>PHYSICAL OUTPUT</div><div style={{fontSize:17,fontFamily:"monospace",fontWeight:800,color,marginTop:2}}>{airflow} m³/s</div><div style={{fontSize:7,color:"#626D72"}}>fan mode {mode}</div></div></div>
    <div style={{marginTop:7,border:"1px solid #343A3E",borderRadius:9,padding:8,background:"#0D1012"}}><div style={{fontSize:6.5,color:"#8D969B",letterSpacing:".12em",marginBottom:6}}>OPERATOR CONTROLS</div><div style={{display:"flex",gap:5}}><button disabled={pending||manual} onClick={()=>aeolus.fire("force-boost")} style={{flex:1,padding:"7px",borderRadius:6,border:"1px solid #69512E",background:"#21180B",color:"#E4B96A",fontSize:7.5,cursor:"pointer"}}>Force boost</button><button disabled={pending||!manual} onClick={()=>aeolus.fire("return-auto")} style={{flex:1,padding:"7px",borderRadius:6,border:"1px solid #365946",background:"#102019",color:"#84D7A0",fontSize:7.5,cursor:"pointer"}}>Return to auto</button></div><div style={{fontSize:6.8,color:"#626B70",marginTop:6}}>Automatic boost requests come over the Aeolus event bus from Atmospheric Safety. This pane owns the fan actuator.</div></div>
    <div style={{fontSize:7,color:"#60696D",marginTop:6,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{action}</div>
  </div>;
}`;
