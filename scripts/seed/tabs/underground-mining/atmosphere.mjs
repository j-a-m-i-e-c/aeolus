  ...
  return <div style={{padding:11,minHeight:"100%",background:"linear-gradient(180deg,#0B0D0F,#080A0C)",color:"#EDF1F4"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1.15fr .85fr",gap:7}}>
      <div style={{border:"1px solid #303438",borderRadius:10,background:"#0B0D0F",padding:9}}>
        {[{name:"LEVEL 3",v:l3},{name:"DRIFT 7",v:d7}].map((g:any)=><div key={g.name} style={{marginBottom:10}}><div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"#929AA0"}}><span>{g.name}</span><span style={{fontFamily:"monospace",color:g.v>=1?"#FF7868":g.v>=.5?"#F2B65B":"#B8C3C8"}}>CH₄ {g.v.toFixed(2)}%</span></div><div style={{height:9,marginTop:4,background:"#191C1E",borderRadius:6,overflow:"hidden",position:"relative"}}><div style={{height:"100%",width:gasWidth(g.v)+"%",background:g.v>=1?"linear-gradient(90deg,#8C322C,#E76855)":g.v>=.5?"linear-gradient(90deg,#6F5125,#D6A149)":"linear-gradient(90deg,#28513A,#57B879)"}}/><div style={{position:"absolute",left:"66.6%",top:0,bottom:0,width:1,background:"#925047"}}/></div></div>)}
        <div style={{fontSize:11,color:"#5F666B"}}>Alarm line shown at 1.00% CH₄. The sensor values are simulator-owned physical MQTT state.</div>
      </div>
      <div style={{border:"1px solid "+(alarm?"#663B34":"#303438"),borderRadius:10,background:alarm?"#160F0D":"#0B0D0F",padding:9}}>
        <div style={{fontSize:11,color:"#7F878C",letterSpacing:".12em"}}>DRIFT 7 PANEL</div><div style={{fontSize:24,fontFamily:"monospace",fontWeight:850,color,marginTop:3}}>{d7.toFixed(2)}%</div><div style={{fontSize:11,color:"#6E7479"}}>methane</div>
    ...
    <div style={{marginTop:7,border:"1px solid #343A3D",borderRadius:9,padding:8,background:"#0D1012"}}><div style={{fontSize:11,color:"#8B9499",letterSpacing:".12em",marginBottom:6}}>OPERATOR CONTROLS</div><button disabled={!alarm||ack} onClick={()=>aeolus.fire("acknowledge-alarm")} style={{width:"100%",padding:"7px",borderRadius:6,border:"1px solid "+(alarm&&!ack?"#70463B":"#34393C"),background:alarm&&!ack?"#241410":"#131618",color:alarm&&!ack?"#F2977E":"#687176",fontSize:11,cursor:"pointer"}}>{alarm?(ack?"Alarm acknowledged":"Acknowledge atmospheric alarm"):"No active alarm"}</button></div>
    <div style={{marginTop:7,border:"1px dashed #6D5232",borderRadius:9,padding:8,background:"#171209"}}><div style={{fontSize:11,color:"#D5B26B",letterSpacing:".12em"}}>DEMO SCENARIO</div><div style={{fontSize:11,color:"#857153",margin:"3px 0 6px"}}>Inject a transient methane pocket into Drift 7. Aeolus should demand ventilation without the demo button commanding a fan.</div><div style={{display:"flex",gap:5}}><button disabled={alarm} onClick={()=>aeolus.fire("simulate-gas-rise")} style={{flex:1,padding:"6px",borderRadius:6,border:"1px solid #6A4A2D",background:"#24170B",color:"#E7B668",fontSize:11,cursor:"pointer"}}>Inject methane pocket</button><button onClick={()=>aeolus.fire("reset-atmosphere")} style={{padding:"6px 9px",borderRadius:6,border:"1px solid #454038",background:"#171713",color:"#919087",fontSize:11,cursor:"pointer"}}>Reset</button></div></div>
