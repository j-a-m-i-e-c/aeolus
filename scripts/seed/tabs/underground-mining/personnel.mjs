...
export default function PersonnelMuster(aeolus: CustomComponentProps){
  ...
  return <div style={{padding:11,minHeight:"100%",background:"linear-gradient(180deg,#0A0C0E,#07090A)",color:"#EDF1F3"}}>
    ...
    <div style={{border:"1px solid #303438",borderRadius:10,background:"#0B0D0F",overflow:"hidden"}}><svg width="100%" height="190" viewBox="0 0 420 190"><rect width="420" height="190" fill="#0B0D0F"/>{[[34,"LEVEL 1"],[86,"LEVEL 2"],[138,"LEVEL 3"]].map((x:any)=><g key={x[1]}><path d={"M45 "+x[0]+" H286"} stroke="#353B3E" strokeWidth="18"/><path d={"M45 "+x[0]+" H286"} stroke="#5A6266" strokeWidth="1.5"/><text x="16" y={x[0]+3} fill="#727A7F" fontSize="6">{x[1]}</text></g>)}<path d="M286 138 H330 V164 H374" fill="none" stroke="#353B3E" strokeWidth="18"/><path d="M286 138 H330 V164 H374" fill="none" stroke="#5A6266" strokeWidth="1.5"/><rect x="332" y="143" width="72" height="40" rx="6" fill={active?"#122218":"#111416"} stroke={active?"#4A9A64":"#42494D"}/><text x="368" y="157" textAnchor="middle" fill="#899398" fontSize="6">REFUGE</text><text x="368" y="174" textAnchor="middle" fill={statusColor} fontSize="13" fontFamily="monospace" fontWeight="800">{refuge}/{underground}</text>
      ...
      <text x="47" y="178" fill="#60686D" fontSize="6.5">L1 {l1} · L2 {l2} · L3 {l3}</text></svg></div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:5,marginTop:6}}>{[["UNDERGROUND",underground],["REFUGE",refuge],["UNACCOUNTED",unaccounted]].map((x:any)=><div key={x[0]} style={{border:"1px solid #303438",borderRadius:7,padding:6,background:"#0C0F10"}}><div style={{fontSize:5.8,color:"#6F777B"}}>{x[0]}</div><div style={{fontSize:15,fontWeight:850,fontFamily:"monospace",color:x[0]==="UNACCOUNTED"&&x[1]>0?"#F07B68":"#C5CFD3",marginTop:1}}>{x[1]}</div></div>)}</div>
    <div style={{marginTop:7,border:"1px solid #343A3D",borderRadius:9,padding:8,background:"#0D1012"}}><div style={{fontSize:6.5,color:"#8C9599",letterSpacing:".12em",marginBottom:6}}>OPERATOR CONTROLS</div><div style={{display:"flex",gap:5}}><button disabled={pending||active} onClick={()=>aeolus.fire("initiate-muster")} style={{flex:1,padding:"7px",borderRadius:6,border:"1px solid #704638",background:"#251410",color:"#EF8D76",fontSize:7.5,cursor:"pointer"}}>Initiate muster</button><button disabled={pending||!active} onClick={()=>aeolus.fire("clear-muster")} style={{flex:1,padding:"7px",borderRadius:6,border:"1px solid #3A5745",background:"#102018",color:"#82D39D",fontSize:7.5,cursor:"pointer"}}>Clear muster</button></div></div>
    <div style={{marginTop:7,border:"1px dashed #6B5131",borderRadius:9,padding:8,background:"#171209"}}><div style={{fontSize:6.5,color:"#D4B16A",letterSpacing:".12em"}}>DEMO SCENARIO</div><div style={{fontSize:7,color:"#847052",margin:"3px 0 6px"}}>Simulate a tracking exception without changing the operator muster state.</div><div style={{display:"flex",gap:5}}><button disabled={unaccounted>0} onClick={()=>aeolus.fire("simulate-tag-dropout")} style={{flex:1,padding:"6px",borderRadius:6,border:"1px solid #6A4D30",background:"#22170B",color:"#E2B56A",fontSize:7.5,cursor:"pointer"}}>Drop one tag</button><button onClick={()=>aeolus.fire("reset-personnel")} style={{padding:"6px 9px",borderRadius:6,border:"1px solid #454139",background:"#171713",color:"#929087",fontSize:7.5,cursor:"pointer"}}>Reset</button></div></div>
    <div style={{fontSize:7,color:"#60686C",marginTop:6,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{action}</div>
  </div>;
}`;
