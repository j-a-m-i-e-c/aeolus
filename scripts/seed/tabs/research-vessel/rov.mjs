...
export default function RovOperations(aeolus: CustomComponentProps) {
  ...
  return <div style={{padding:11,minHeight:"100%",background:"linear-gradient(180deg,#061219,#041018)",color:"#EDF3F5"}}>
    ...
    <div style={{marginTop:7,border:"1px solid #263F4A",borderRadius:9,padding:8,background:"#07151D"}}><div style={{color:"#80949D",fontSize:11,letterSpacing:".12em",marginBottom:6}}>OPERATOR CONTROLS</div><div style={{display:"flex",gap:5,flexWrap:"wrap"}}><button disabled={pending||moving} onClick={()=>aeolus.fire("rov-dive")} style={{flex:1,minWidth:75,padding:"7px",borderRadius:6,border:"1px solid #2B5E70",background:"#0B2632",color:"#72D9EF",fontSize:11,cursor:"pointer"}}>Dive 360 m</button><button disabled={pending||moving} onClick={()=>aeolus.fire("rov-survey")} style={{flex:1,minWidth:75,padding:"7px",borderRadius:6,border:"1px solid #315B43",background:"#102219",color:"#83DA9C",fontSize:11,cursor:"pointer"}}>Start transect</button><button disabled={pending} onClick={()=>aeolus.fire("rov-hold")} style={{padding:"7px 9px",borderRadius:6,border:"1px solid #454A4C",background:"#171A1C",color:"#AAB4B8",fontSize:11,cursor:"pointer"}}>Hold</button><button disabled={pending||moving} onClick={()=>aeolus.fire("rov-recover")} style={{padding:"7px 9px",borderRadius:6,border:"1px solid #5B4931",background:"#211A10",color:"#DDBB7B",fontSize:11,cursor:"pointer"}}>Recover</button></div></div>
    <div style={{marginTop:7,border:"1px dashed #69502E",borderRadius:9,padding:8,background:"#171309"}}><div style={{color:"#D8B66D",fontSize:11,letterSpacing:".12em"}}>DEMO SCENARIO</div><div style={{color:"#806F50",fontSize:11,margin:"3px 0 6px"}}>Inject a deep cross-current. High tether load should make Aeolus command a safe hold.</div><div style={{display:"flex",gap:5}}><button onClick={()=>aeolus.fire("simulate-rov-current")} style={{flex:1,padding:"6px",borderRadius:6,border:"1px solid #6A5130",background:"#21180B",color:"#E3B866",fontSize:11,cursor:"pointer"}}>Inject cross-current</button><button onClick={()=>aeolus.fire("reset-rov")} style={{padding:"6px 9px",borderRadius:6,border:"1px solid #454138",background:"#171713",color:"#898B82",fontSize:11,cursor:"pointer"}}>Reset mission</button></div></div>
