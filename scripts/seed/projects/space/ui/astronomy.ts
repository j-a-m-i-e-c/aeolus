...
export function nightShape(sub:{lat:number;lon:number}){const RAD=Math.PI/180;const px=(lon:number)=>((lon+180)/360)*480,py=(lat:number)=>18+((90-Math.max(-90,Math.min(90,lat)))/180)*240;let t=Math.tan(sub.lat*RAD);if(Math.abs(t)<1e-6)t=t<0?-1e-6:1e-6;const pts:string[]=[];for(let lon=-180;lon<=180;lon+=2){const H=(lon-sub.lon)*RAD;pts.push(px(lon).toFixed(2)+" "+py(Math.atan(-Math.cos(H)/t)/RAD).toFixed(2));}const edge=(t>0?py(-90):py(90)).toFixed(2);return {area:"M"+pts.join(" L")+" L480 "+edge+" L0 "+edge+" Z",line:"M"+pts.join(" L")};}
