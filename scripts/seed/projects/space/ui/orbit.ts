...
export function wrapLon(lon:number){var x=lon;while(x>180)x-=360;while(x<-180)x+=360;return x;}
...
export function issStep(u0:number,lon0:number,dtSeconds:number,periodS:number){const u=u0+(dtSeconds/periodS)*360;let dn=issNode(u)-issNode(u0);while(dn>180)dn-=360;while(dn<-180)dn+=360;return {u:u,lat:issLat(u),lon:lon0+dn-EARTH_ROT_DEG_S*dtSeconds};}
...
export function issArc(u0:number,lon0:number,periodS:number,stepS:number,steps:number,dir:number){const pts=[{lat:issLat(u0),lon:lon0}];let u=u0,lon=lon0;for(let s=1;s<=steps;s++){const n=issStep(u,lon,dir*stepS,periodS);u=n.u;lon=n.lon;pts.push({lat:n.lat,lon:n.lon});}return dir<0?pts.reverse():pts;}
...
export function issPolylines(pts:{lat:number;lon:number}[],px:(lon:number)=>number,py:(lat:number)=>number){return issSegments(pts).map(function(seg){return seg.map(function(p){return px(p.lon).toFixed(1)+","+py(p.lat).toFixed(1)}).join(" ")});}
...
// one boundary.
export function issSegments(pts:{lat:number;lon:number}[]){const segs:{lat:number;lon:number}[][]=[];let cur:{lat:number;lon:number}[]=[];for(let i=0;i<pts.length;i++){if(i>0){const q=pts[i-1],p=pts[i],lo=Math.min(q.lon,p.lon),hi=Math.max(q.lon,p.lon),L=180+360*Math.ceil((lo-180)/360);if(p.lon!==q.lon&&L>lo&&L<=hi){const f=(L-q.lon)/(p.lon-q.lon),lat=q.lat+f*(p.lat-q.lat),east=p.lon>q.lon;cur.push({lat:lat,lon:east?180:-180});segs.push(cur);cur=[{lat:lat,lon:east?-180:180}];}}cur.push(pts[i]);}segs.push(cur);return segs.filter(function(s){return s.length>1;});}
