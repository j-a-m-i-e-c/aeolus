    ...
    <div style={{ padding: 12, minHeight: "100%", background: "linear-gradient(180deg,#10120C,#0B0E0A)", color: "#EDEFE8" }}>
      ...
      <div style={{ marginTop: 7, padding: 8, border: "1px dashed #615034", borderRadius: 9, background: "#19140D" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.4fr .9fr .65fr", gap: 6 }}>
          <button onClick={() => aeolus.fire("simulate-low-battery")} disabled={!available || !!demoScenarioPending} style={{ borderRadius: 8, padding: "8px", border: "1px solid #6C4A2F", background: "#25180F", color: !available || demoScenarioPending ? "#735E4C" : "#E5A268", fontSize:11, fontWeight: 750, cursor: !available || demoScenarioPending ? "not-allowed" : "pointer" }}>Cloud + low reserve</button>
          <button onClick={() => aeolus.fire("restore-battery")} disabled={available || !!demoScenarioPending} style={{ borderRadius: 8, padding: "8px", border: "1px solid #315B3C", background: "#102319", color: available || demoScenarioPending ? "#516A57" : "#83D99A", fontSize:11, cursor: available || demoScenarioPending ? "not-allowed" : "pointer" }}>Restore nominal</button>
          <button onClick={() => aeolus.fire("reset-energy")} style={{ borderRadius: 8, padding: "8px", border: "1px solid #433B30", background: "#181510", color: "#8E8678", fontSize:11, cursor: "pointer" }}>Reset demo</button>
        </div>
      </div>
