...
// Uses a custom tab to access the automations page (automations live as panes
// inside tabs, but also the standalone /automations concept via API).
...
test.describe("automations", () => {
  test("can create a form-based automation rule via API and see it listed", async ({ page }) => {
    ...
    // Create a rule via the API to guarantee one exists
    const ruleName = `E2E Rule ${Date.now()}`;
    const createRes = await page.request.post(`${API_URL}/api/automations`, {
      data: {
        name: ruleName,
    ...
    const listRes = await page.request.get(`${API_URL}/api/automations`);
    expect(listRes.ok()).toBe(true);
    const rules = (await listRes.json()) as Array<{ id: string; name: string }>;
    expect(rules.some((r) => r.id === ruleId)).toBe(true);

    // Clean up
    await page.request.delete(`${API_URL}/api/automations/${ruleId}`);
  });
  ...
  test("can toggle an automation rule on and off", async ({ page }) => {
    ...
    // Disable the rule
    const disableRes = await page.request.patch(`${API_URL}/api/automations/${ruleId}/toggle`, {
      data: { enabled: false },
    });
    ...
    // Re-enable the rule
    const enableRes = await page.request.patch(`${API_URL}/api/automations/${ruleId}/toggle`, {
      data: { enabled: true },
    });
  ...
  test("can delete an automation rule", async ({ page }) => {
    ...
    // Delete the rule
    const deleteRes = await page.request.delete(`${API_URL}/api/automations/${ruleId}`);
  ...
  test("can manually fire an automation rule", async ({ page }) => {
    ...
    const fireRes = await page.request.post(`${API_URL}/api/automations/${ruleId}/fire`);
    ...
    const historyRes = await page.request.get(`${API_URL}/api/automations/history?ruleId=${ruleId}`);
