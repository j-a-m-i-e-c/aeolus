...
`scripts/deploy/bootstrap-demo-host.sh`.
...
When Terraform has been initialized in `infra/public-demo`, the script can read
...
7. installs/enables the nightly reset timer;
8. rolls back to the previous deployment source + image configuration if the release
   health gate fails.
...
```bash
./scripts/deploy/seed-demo-golden-remote.sh
```

It opens a TTY on the host and prompts there for the seed-admin password, so the password is not put in local shell history. The equivalent host-side command is `./scripts/deploy/seed-demo-and-create-golden.sh`.
...
docker compose -f docker-compose.public-demo.yml ps
```

Logs:

```bash
docker compose -f docker-compose.public-demo.yml logs --tail=100 backend simulator cloudflared
```
