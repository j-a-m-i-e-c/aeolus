...
- [ ] 1. Create backend data layer and state store
  - [ ] 1.1 Add `custom_panels` and `panel_state` tables to database schema
  ...
  - [ ] 1.2 Create `PanelStateStore` class in `src/panels/panel-state-store.ts`
...
- [ ] 2. Create panel type definitions and event bus extension
  - [ ] 2.1 Create `CustomPanelProps` type definition in `src/panels/panel-types.d.ts`
  ...
  - [ ] 2.2 Add `PANEL_STATE_CHANGE` event to `src/core/event-bus.ts`
    - Export `PANEL_STATE_CHANGE = "panel:state-change"` constant
    - _Requirements: 11.1, 11.3_

- [ ] 3. Implement panel CRUD API routes
  - [ ] 3.1 Create `src/api/routes/panel.routes.ts` with all endpoints
...
- [ ] 4. Wire backend into application entry point
  - [ ] 4.1 Register panel routes and state store in `src/index.ts`
...
- [ ] 6. Create frontend panel state store
  - [ ] 6.1 Create Zustand store in `frontend/src/store/panel-state-store.ts`
...
- [ ] 7. Adapt `useDynamicComponent` hook and add WebSocket handler
  - [ ] 7.1 Parameterize `useDynamicComponent` to accept a `moduleUrl` argument
  ...
  - [ ] 7.2 Add `panel-state` message handler to WebSocket client
...
- [ ] 8. Create CustomPanelPane component
  - [ ] 8.1 Create `frontend/src/components/panes/CustomPanelPane.tsx`
...
- [ ] 9. Register pane type and update PanePicker
  - [ ] 9.1 Add `custom-panel` entry to pane registry
  ...
  - [ ] 9.2 Exclude `custom-panel` from PanePicker and rename header
...
- [ ] 10. Update TabLayout with "New Pane" button and rename "Add Pane"
  - [ ] 10.1 Add "New Pane" button and rename "Add Pane" to "Browse Panes" in `frontend/src/components/TabLayout.tsx`
  ...
  - [ ] 10.2 Implement panel deletion on pane removal
