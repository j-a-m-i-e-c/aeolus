...
[ -n "${DEMO_SSH_HOST:-}" ] || die "DEMO_SSH_HOST is required (or apply demo/infrastructure/terraform with Terraform first)"
...
if [ "$MODE" = "registry" ]; then
  ...
  ssh "${ssh_args[@]}" "$remote" "cd '${DEMO_APP_DIR}' && docker compose --project-directory . -f demo/compose/hosted-runtime.yml pull backend frontend simulator"
fi

log "Starting release without compiling on the VM…"
if ! ssh "${ssh_args[@]}" "$remote" "cd '${DEMO_APP_DIR}' && docker compose --project-directory . -f demo/compose/hosted-runtime.yml up -d --remove-orphans && ./demo/operations/health-check.sh"; then
...
ssh "${ssh_args[@]}" "$remote" "cd '${DEMO_APP_DIR}' && sudo cp demo/operations/systemd/aeolus-demo-reset.service demo/operations/systemd/aeolus-demo-reset.timer /etc/systemd/system/ && sudo systemctl daemon-reload"
