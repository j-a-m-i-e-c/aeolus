// wildlife-detection — visual implementation behind ui/index.tsx
import { useEffect, useState } from "react";
import { formatNumber, percent, watts } from "@aeolus/ui";
function Animal({ kind, x, y, predator }: {
    kind: string;
    x: number;
    y: number;
    predator: boolean;
}) { const c = predator ? "#E67B5D" : "#C8D5C5"; if (kind === "red-fox")
    return <g transform={"translate(" + x + " " + y + ")"} fill={c} stroke={c}><ellipse rx="28" ry="11"/><circle cx="25" cy="-8" r="9"/><path d="M19 -15 L23 -27 L28 -16 M29 -16 L36 -26 L35 -12"/><path d="M-25 -2 Q-54 -20 -63 -3 Q-48 4 -28 7" fill="none" strokeWidth="9" strokeLinecap="round"/><line x1="-14" y1="9" x2="-17" y2="24" strokeWidth="4"/><line x1="14" y1="9" x2="17" y2="24" strokeWidth="4"/></g>; if (kind === "feral-cat")
    return <g transform={"translate(" + x + " " + y + ")"} fill={c} stroke={c}><ellipse rx="22" ry="9"/><circle cx="20" cy="-8" r="8"/><path d="M14 -14 L17 -24 L22 -15 M22 -15 L28 -23 L28 -11"/><path d="M-20 -4 Q-43 -18 -42 -34" fill="none" strokeWidth="5"/><line x1="-10" y1="7" x2="-12" y2="22" strokeWidth="4"/><line x1="11" y1="7" x2="13" y2="22" strokeWidth="4"/></g>; if (kind === "echidna")
    return <g transform={"translate(" + x + " " + y + ")"} fill={c} stroke={c}><path d="M-28 7 Q-22 -16 5 -17 Q25 -16 30 3 Q18 15 -6 15 Q-21 15 -28 7 Z"/>{[-20, -12, -4, 4, 12, 20].map(v => <line key={v} x1={v} y1="-10" x2={v - 5} y2="-26" strokeWidth="2"/>)}<path d="M27 0 L43 5 L29 8"/></g>; if (kind === "lyrebird")
