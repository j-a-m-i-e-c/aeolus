...
export default function GameMasterConsole({ model, actions }: {
    ...
    const solved = Number(model.solved ?? 0), p = [Boolean(model.p1), Boolean(model.p2), Boolean(model.p3), Boolean(model.p4)], base = Number(model.remaining ?? 2700), started = Number(model.timerStartedAt ?? 0), status = String(model.status || "ready"), exit = Boolean(model.exitUnlocked), currentRoom = String(model.currentRoom || "Library"), hints = Number(model.hintsSent ?? 0), hint = String(model.lastHint || "No hint sent yet."), hintId = Number(model.lastHintId ?? 0), hintRoom = String(model.hintRoom || "Library"), pending = Boolean(model.pendingHint), talking = Boolean(model.intercomTx), intercomPending = Boolean(model.intercomPending), look = String(model.requestedLook || "puzzle"), last = model.lastAction as any;
    const [now, setNow] = useState(Date.now());
    useEffect(() => { const id = setInterval(() => setNow(Date.now()), 250); return () => clearInterval(id); }, []);
    // The same derivation the Logic uses: seconds recorded at the last transition, less
    // the time since, and only while the clock is running. READY and PAUSED do not tick,
    // so sitting on this pane cannot burn a session.
    const running = status === "running";
    const remaining = running && started > 0
        ? Math.max(0, base - Math.floor((now - started) / 1000))
        : base;
    const clock = String(Math.floor(remaining / 60)).padStart(2, "0") + ":" + String(remaining % 60).padStart(2, "0");
    // Time can run out between two invocations of the Logic, so the pane recognises it
    // immediately and the recorded status catches up when the automation next runs.
    const timeUp = status === "expired" || (running && remaining === 0);
    const idle = status === "ready" || status === "completed" || timeUp;
    const statusLabel = timeUp ? "TIME EXPIRED"
        : status === "ready" ? "READY"
            : status === "paused" ? "PAUSED"
                : status === "completed" ? "ESCAPED · " + clock + " LEFT"
                    : solved + "/4 solved · team in " + currentRoom;
    const session = (e: string) => actions.session(e);
    const hintAction = (e: string) => actions.hint(e);
    const roomLook = (e: string) => actions.roomLook(e);
    ...
    return <div style={{ padding: 14, minHeight: "100%", background: "linear-gradient(180deg,#0D0B10,#08070A)", color: "#F0EBF3" }}><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10, gap: 12 }}><div><div style={{ fontSize: 18, fontWeight: 900 }}>GAME MASTER</div><div style={{ fontSize: 12, color: "#8E8393", marginTop: 3 }}>Session clock · hints · intercom · room looks · verified exit release</div></div><div style={{ textAlign: "right" }}><div style={{ fontFamily: "monospace", fontSize: 25, fontWeight: 900, color: timeUp ? "#F06E69" : status === "completed" ? "#76DB98" : idle ? "#9C93A2" : remaining < 300 ? "#F06E69" : "#76DB98" }}>{clock}</div><div style={{ fontSize: 11, color: "#857A89" }}>{statusLabel}</div></div></div>
