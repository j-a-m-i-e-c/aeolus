// bunker-perimeter Ã¢â‚¬â€ visual implementation behind ui/index.tsx
...
export default function PerimeterSecurityPanel({ model, actions }: {
    ...
    // The tracked group, which is not the same number as the alert count: something
    // walking in from the treeline exists and has a position before it crosses the ring.
 ...
 return <div style={{ padding: 13, minHeight: "100%", background: "#0B0D09", color: "#EEF0E9" }}><div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}><div><div style={{ fontSize: 17, fontWeight: 900 }}>PERIMETER SECURITY</div><div style={{ fontSize: 12, color: "#858D82", marginTop: 3 }}>Local classification Ã¢â€ â€™ policy Ã¢â€ â€™ verified floodlighting</div></div><div style={{ textAlign: "right" }}><div style={{ fontSize: 12, fontWeight: 850, color: threat ? "#ED8B72" : withdrawing ? "#D9C46A" : "#7FD19A" }}>{headline}</div><div style={{ fontSize: 11, color: auto ? "#83C99A" : "#D9B06A", marginTop: 2 }}>LIGHTING {auto ? "AUTO" : "MANUAL"}</div></div></div>
    <div style={{ border: "1px solid #34382F", borderRadius: 11, background: "#10120D", marginTop: 9, overflow: "hidden" }}><svg width="100%" height="185" viewBox="0 0 430 170"><rect width="430" height="170" fill="#11150F"/>
      ...
      {Array.from({ length: Math.min(approaching, 4) }).map((_, i) => <Z key={i} x={xFor(range, i % 2 === 0 ? -1 : 1) + Math.floor(i / 2) * 17 * (i % 2 === 0 ? 1 : -1)} y={132} p={p + i} lit={beamPct > 40}/>)}
      <text x="215" y="158" textAnchor="middle" fill="#899184" fontSize="10">{sector.toUpperCase()} SECTOR Ã‚Â· FLOODLIGHTS {percent(beamPct)} Ã‚Â· {auto ? "AUTO" : "MANUAL OVERRIDE"}</text></svg></div>
    ...
    <div style={{ marginTop: 8, border: "1px solid #373A33", borderRadius: 10, padding: 9, background: "#0F110E" }}><div style={{ fontSize: 11, color: "#9BA097", letterSpacing: ".1em", marginBottom: 7 }}>OPERATOR CONTROLS</div><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}><button {...lightVisual} style={{ ...lightVisual.style, padding: "9px" }} onClick={() => actions.toggleLights()}>{!available ? "Floodlight controller offline" : pending ? "Verifying floodlight commandÃ¢â‚¬Â¦" : lights ? "Turn floodlights off" : "Turn floodlights on"}</button><button {...autoVisual} style={{ ...autoVisual.style, padding: "9px" }} onClick={() => actions.returnAuto()}>{auto ? "AUTO policy active" : "Return to automatic control"}</button></div><div style={{ fontSize: 11, color: "#7D847A", marginTop: 7 }}>The button reflects the floodlights Aeolus last OBSERVED, not what it asked for. A verified manual command enters MANUAL override; Return to AUTO hands the lights back to contact policy.</div></div>
