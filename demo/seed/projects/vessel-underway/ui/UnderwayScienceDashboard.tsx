// vessel-underway — visual implementation behind ui/index.tsx
import { useMemo } from "react";
import { CommandProofCard, control } from "@aeolus/ui";
export default function UnderwayScienceDashboard({ model, actions }: {
    model: Record<string, any>;
    actions: Record<string, (...args: any[]) => void>;
}) {
    const sst = Number(model.sst ?? 18.4), sal = Number(model.salinity ?? 35.2), flow = Number(model.flow ?? 2.1), chl = Number(model.chlorophyll ?? .8), turb = Number(model.turbidity ?? .5);
    const pumpOn = Boolean(model.pumpOn), front = Boolean(model.frontDetected), pending = Boolean(model.commandPending);
    const profile = (model.profile as any[]) || [];
    const last = model.lastAction as any;
    const action = last?.label ? String(last.label) : "Surface-water stream online";
    const pts = profile.length ? profile : [{ sst: 18.4, salinity: 35.2 }, { sst: 18.35, salinity: 35.18 }, { sst: 18.4, salinity: 35.2 }];
    // Only one operator action is ever available, so a pending request is
    // unambiguous: with the intake running it can only be the stop.
    const starting = pending && !pumpOn, stopping = pending && pumpOn;
    const startVisual = control({ pending: starting, current: pumpOn });
    const stopVisual = control({ pending: stopping, disabled: !pumpOn });
    const tempPath = useMemo(() => pts.map((p: any, i: number) => { const x = 18 + i * Math.max(1, 245 / (Math.max(1, pts.length - 1))); const y = 112 - (Number(p.sst) - 15) * 15; return (i ? "L" : "M") + x.toFixed(1) + "," + y.toFixed(1); }).join(" "), [profile]);
    const salPath = useMemo(() => pts.map((p: any, i: number) => { const x = 18 + i * Math.max(1, 245 / (Math.max(1, pts.length - 1))); const y = 112 - (Number(p.salinity) - 34.6) * 45; return (i ? "L" : "M") + x.toFixed(1) + "," + y.toFixed(1); }).join(" "), [profile]);
    ...
    <div style={{ marginTop: 7, border: "1px solid #294442", borderRadius: 9, padding: 8, background: "#081618" }}><div style={{ color: "#809592", fontSize: 11, letterSpacing: ".12em", marginBottom: 6 }}>OPERATOR CONTROLS</div><div style={{ display: "flex", gap: 5 }}><button {...startVisual} style={{ ...startVisual.style, flex: 1 }} onClick={() => actions.samplingStart()}>{starting ? "Starting intake pump…" : pumpOn ? "Sampling" : "Start sampling"}</button><button {...stopVisual} style={{ ...stopVisual.style, flex: 1 }} onClick={() => actions.samplingStop()}>{stopping ? "Stopping intake pump…" : "Stop sampling"}</button></div></div>
    {/* Every reading on this pane is taken from water the intake pump is drawing, so
        whether flow was actually measured is the question behind all of them. The
        receipt names the flow meter as a separate instrument from the pump. */}
    <CommandProofCard evidence={model.lastCommand} label="Last command"/>
    <div style={{ marginTop: 16, border: "1px dashed #69502E", borderRadius: 9, padding: 8, background: "#171309" }}><div style={{ color: "#D8B66D", fontSize: 11, letterSpacing: ".12em" }}>DEMO SCENARIO</div><div style={{ color: "#806F50", fontSize: 11, margin: "3px 0 6px" }}>Move the simulated vessel across a water-mass boundary. Aeolus detects the gradient from real stream telemetry.</div><div style={{ display: "flex", gap: 5 }}><button disabled={!pumpOn} onClick={() => actions.simulateFront()} style={{ flex: 1, padding: "6px", borderRadius: 6, border: "1px solid #6A5130", background: "#21180B", color: "#E3B866", fontSize: 11, cursor: "pointer" }}>Cross hydrographic front</button><button onClick={() => actions.resetUnderway()} style={{ padding: "6px 9px", borderRadius: 6, border: "1px solid #454138", background: "#171713", color: "#898B82", fontSize: 11, cursor: "pointer" }}>Reset transect</button></div></div>
