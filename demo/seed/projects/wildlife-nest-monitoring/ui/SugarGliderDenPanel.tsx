...
export default function SugarGliderDenPanel({ model, actions }: {
    ...
    <div style={{ marginTop: 9, border: "1px solid #373D35", borderRadius: 10, padding: 9, background: "#0D100D" }}><div style={{ fontSize: 11, color: "#A0A89F", letterSpacing: ".1em", marginBottom: 7 }}>OPERATOR CONTROLS</div><div style={{ display: "flex", gap: 6 }}><button {...autoVisual} style={{ ...autoVisual.style, flex: 1, padding: "9px" }} onClick={() => actions.toggleAutoCooling()}>{pending ? "Verifying…" : auto ? "Automatic cooling armed" : "Automatic cooling off"}</button><button {...stopVisual} style={{ ...stopVisual.style, flex: 1, padding: "9px" }} onClick={() => actions.stopCooling()}>Stop den fan</button></div><div style={{ fontSize: 11, color: "#767D75", marginTop: 7 }}>{outcome}</div></div>
