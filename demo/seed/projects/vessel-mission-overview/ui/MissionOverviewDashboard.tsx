...
export default function MissionOverviewDashboard({ model }: {
    ...
    const sst = Number(model.sst ?? 18.4);
    const surfaceSalinity = Number(model.surfaceSalinity ?? 35.2);
    const chlorophyll = Number(model.chlorophyll ?? .8);
    const front = Boolean(model.frontDetected);
    const last = model.lastMissionEvent as any;
