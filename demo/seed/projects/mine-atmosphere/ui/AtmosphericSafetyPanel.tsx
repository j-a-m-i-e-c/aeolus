...
export default function AtmosphericSafetyPanel({ model, actions }: {
    ...
    const l3 = Number(model.l3Ch4 ?? .30), d7 = Number(model.d7Ch4 ?? .42), co = Number(model.co ?? 16), o2 = Number(model.o2 ?? 20.7), no2 = Number(model.no2 ?? 1.6);
    const severity = String(model.severity || "safe"), alarm = Boolean(model.alarm), ack = Boolean(model.acknowledged), demand = Number(model.ventDemand ?? 48);
    ...
    <div style={{ marginTop: 7, border: "1px dashed #6D5232", borderRadius: 9, padding: 8, background: "#171209" }}><div style={{ fontSize: 11, color: "#D5B26B", letterSpacing: ".12em" }}>DEMO SCENARIO</div><div style={{ fontSize: 11, color: "#857153", margin: "3px 0 6px" }}>Inject a transient methane pocket into Drift 7. Aeolus should demand ventilation without the demo button commanding a fan.</div><div style={{ display: "flex", gap: 5 }}><button disabled={alarm} onClick={() => actions.simulateGasRise()} style={{ flex: 1, padding: "6px", borderRadius: 6, border: "1px solid #6A4A2D", background: "#24170B", color: "#E7B668", fontSize: 11, cursor: "pointer" }}>Inject methane pocket</button><button onClick={() => actions.resetAtmosphere()} style={{ padding: "6px 9px", borderRadius: 6, border: "1px solid #454038", background: "#171713", color: "#919087", fontSize: 11, cursor: "pointer" }}>Reset</button></div></div>
