// vessel-ctd â€” visual implementation behind ui/index.tsx
import { useEffect, useState } from "react";
import { CommandProofCard, control, decimal, integer, metres, salinity as psu, temperature } from "@aeolus/ui";
...
export default function CtdOperationsDashboard({ model, actions }: {
    ...
    const action = last?.label ? String(last.label) : "CTD on deck Â· ready to deploy";
 ...
 return <div style={{ padding: 11, minHeight: "100%", background: "linear-gradient(180deg,#09131A,#061018)", color: "#EDF2F4" }}>
    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 7 }}><div><div style={{ fontSize: 12, fontWeight: 900 }}>CTD OPERATIONS</div><div style={{ color: "#687C87", fontSize: 11, marginTop: 2 }}>Winch control Â· water-column profile Â· cable-tension protection</div></div><div style={{ textAlign: "right" }}><div style={{ color: tensionHigh ? "#EF8A67" : moving ? "#69D7EF" : onDeck ? "#84DBA2" : "#9FD3B4", fontSize: 11, fontWeight: 850 }}>{tensionHigh ? "TENSION INTERLOCK" : (PHASE_LABEL[status] || status.toUpperCase())}</div><div style={{ color: "#60737D", fontSize: 11 }}>{metres(depth)} Â· target {metres(target)}</div></div></div>
    <div style={{ border: "1px solid #1F3948", borderRadius: 10, background: "#05131C", overflow: "hidden" }}><svg width="100%" height="220" viewBox="0 0 500 220">
      ...
      <line x1="210" y1={y(depth)} x2="474" y2={y(depth)} stroke="#6ED899" strokeDasharray="3 3"/><text x="212" y="208" fill="#F0A84A" fontSize="10">TEMP {temperature(model.temperature)}</text><text x="302" y="208" fill="#51D1EA" fontSize="10">SAL {psu(model.salinity)}</text><text x="404" y="208" fill="#83D29D" fontSize="10">Oâ‚‚ {decimal(model.oxygen, 1)}</text>
    </svg></div>
