...
### Modbus Connector (industrial & energy devices)
Add Modbus support to bring the huge world of industrial and energy hardware — solar inverters, energy meters, VFDs, PLCs, BMS units — into Aeolus. Modbus is register-based request/response (not pub/sub), so a translation layer must map registers to Aeolus's device/state model. **Modbus TCP** is the natural fit: it's plain Node over a TCP socket with no extra hardware on the Pi, and the register-polling model maps cleanly onto the connector framework's `discoverDevices()` loop. Two integration paths: (1) a native TypeScript Modbus TCP connector with a config-driven register map, or (2) an ESP32 / off-the-shelf gateway that bridges Modbus→MQTT, requiring no backend code. Modbus RTU (RS-485) would follow via a USB-RS485 dongle. Start read-only; register writes to live equipment are higher-risk and should be added deliberately.

**Solar inverters (e.g. Deye) as the first target.** A farm-relevant use case: read PV production, battery SoC, grid import/export, and load, then drive solar-aware automations ("battery below 30% and grid price high → shed the hot-water load"). One Deye-specific wrinkle: the WiFi/LAN "Solarman" logger stick exposes TCP port 8899 using the proprietary **Solarman V5** protocol (a Modbus wrapper), *not* standard Modbus TCP — so either implement V5 framing, put an RS-485-to-Ethernet gateway on the inverter's COM port to get real Modbus TCP, or (lowest-effort, read-only) run the mature [`deye-inverter-mqtt`](https://github.com/kbialek/deye-inverter-mqtt) bridge as a container and let Aeolus ingest its MQTT topics with zero backend changes. Register maps for common models are already well documented by the community.
...
Integrate smart locks (Yale, August, Nuki, Schlage, TTLock) for keyless entry control from the Aeolus dashboard. Generate temporary access codes via automation scripts — particularly powerful when combined with the short-term rental automation vertical (auto-generate a unique code per Airbnb booking, expire it at checkout). Surface lock state (locked/unlocked/jammed) in the device registry, log access events in the event log, and trigger automations on lock/unlock (e.g. "when front door unlocks after 6pm, turn on hallway lights"). Many smart locks expose local APIs or work via Zigbee/Z-Wave, fitting naturally into the connector framework.

### Garden & Irrigation Automation
Connect smart irrigation controllers and garden sensors (Holman, Orbit B-hyve, OpenSprinkler, soil moisture sensors via Zigbee/MQTT) for automated watering schedules driven by real data. Combine soil moisture readings, local weather forecasts (via the external services framework), and rain predictions to skip unnecessary watering cycles. Surface zone status, run history, and water usage in a dedicated garden pane. Automation scripts can express rules like "water zone 3 for 15 minutes at 6am, but skip if rain probability exceeds 60% or soil moisture is above 40%." This ties into the energy/cost analytics opportunity — tracking water usage and savings from smart scheduling.
...
### Visual Flow Editor
Drag-and-drop canvas for building automations visually (Node-RED style). Nodes for triggers, conditions, and actions connected by wires. Would generate the same underlying rule structure as the form-based editor and TypeScript scripts, providing a more intuitive way to create complex multi-step automations.
...
These are areas where the current smart home landscape has genuine gaps. Aeolus's architecture — TypeScript automations, pluggable connectors, event bus, local-first design — positions it well to explore any of these directions.
...
Calendar-driven automation for vacation rental properties. Sync with Airbnb/VRBO booking calendars to automatically generate smart lock codes per guest, pre-condition HVAC before check-in, set welcome lighting scenes, switch to energy-saving mode during vacancy, and send automated check-in instructions. The workflow is well-defined but no open source platform handles it end-to-end. Rental Home Automator exists but is basic and closed-source. Aeolus's TypeScript automations are a natural fit for expressing calendar-triggered automation rules, and the connector framework already supports smart locks (via Kasa/Zigbee) and thermostats. This is a vertical with paying customers — hosts managing 5-50 properties would pay for a reliable, self-hosted solution.
...
### On-Device Vision AI & Wildlife Conservation
Pair Aeolus with an on-device AI accelerator — the Raspberry Pi AI Kit / AI HAT+ (Hailo-8/8L NPU) or a Google Coral Edge TPU — to run computer vision models locally at real-time frame rates, with no cloud inference and no footage leaving the device. A small process runs object detection/classification against a camera feed and publishes results to MQTT (`camera/trailcam-01/detection → { species, confidence, box }`), so a detection becomes just another event on the existing bus. No new subsystem is required: the MqttService normalises it, the device registry upserts a `camera` device, and the automation engine evaluates rules against it exactly like a tank level or a door sensor. The inference — the genuinely specialised part — lives on the accelerator behind an MQTT topic; Aeolus treats the model's *output* as data, which is precisely the abstraction it was built around. This pairs naturally with the **Smart Camera Integration** connector item (routing the same detection events through the connector framework instead of raw MQTT).

The standout application is **wildlife monitoring and conservation**, where offline operation and privacy are hard constraints rather than nice-to-haves. Point a solar-powered camera at a nest, burrow, or game trail; the on-site model distinguishes native species from introduced predators; and a detection becomes a logged sighting, a nest-temperature alert, or an instant deterrent trigger — all computed locally, on a fence post, with no cell signal for months. Nothing about an endangered species' location is ever streamed to a third-party cloud. The seed ships a **Wildlife & Conservation** demo tab (trail-cam detections, nest monitor, predator deterrent, biodiversity log) that simulates the full loop, and the [What Is Aeolus?](WHAT_IS_AEOLUS.md) doc frames the use case for a general audience.

### Small Building / Commercial Lite Management
Traditional Building Management Systems (BMS) cost $2.50-8 per square foot and are designed for large commercial buildings. Small businesses, churches, community centres, co-working spaces, and small offices have zero affordable options for HVAC scheduling, occupancy-based lighting, and energy monitoring. Aeolus running on a Pi with a handful of smart plugs, temperature sensors, and occupancy detectors could serve this market at a fraction of the cost. The dashboard's modular pane system already supports multi-room views, and automation scripts can express occupancy-based rules. This is a genuinely underserved segment — IoT for All calls it "the other 90%" of buildings that have no smart infrastructure.
...
The current script automations run inside a secured isolated-vm sandbox with a curated set of globals (`devices`, `automation()`, state, the data store). That keeps untrusted code safe, but it also limits reach — scripts can't freely import npm packages or call arbitrary external APIs. The SDK vision is to give developers full, typed TypeScript access to the entire Aeolus runtime for trusted, code-first automations.
...
// Cron-based scheduling across the whole runtime
